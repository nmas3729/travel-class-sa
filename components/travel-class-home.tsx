...
const enquiryTypes = ['Holiday', 'Flights', 'Accommodation', 'Airport Transfer', 'Coach & Bus Hire', 'Corporate Travel', 'Group Travel', 'Tours & Experiences']
  ...
  return <main id="top">
    <header className="site-header">
      ...
      <nav className="desktop-nav" aria-label="Primary navigation">
        {['Holidays', 'Flights', 'Accommodation', 'Airport Transfers', 'Coach & Bus Hire', 'Corporate Travel', 'Group Travel', 'Tours & Experiences', 'Travel Premium', 'About'].map(item => <a key={item} href={`#${item.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`}>{item}</a>)}
      </nav>
    ...
    {menuOpen && <nav className="mobile-nav" aria-label="Mobile navigation">{['Holidays', 'Flights', 'Accommodation', 'Airport Transfers', 'Coach & Bus Hire', 'Corporate Travel', 'Group Travel', 'Tours & Experiences', 'Travel Premium', 'About'].map(item => <a key={item} onClick={() => setMenuOpen(false)} href={`#${item.toLowerCase().replace(/ & /g, '-').replace(/ /g, '-')}`}>{item}</a>)}<a className="mobile-nav-cta" onClick={() => setMenuOpen(false)} href="#enquiry">Request a quote <ArrowUpRight size={15} /></a></nav>}
    ...
    <section className="services-section" id="transport"><div className="section-label">What we do <span>03—03</span></div><div className="services-head"><div><p className="eyebrow red">One partner. Every detail.</p><h2>Everything<br /><em>in motion.</em></h2></div><p>One trusted team to plan, book and manage every part of your travel. No loose ends. No handovers. Just a better way to go.</p></div><div className="services-editorial"><div className="service-feature"><span>01 / 08</span><h3>Flights</h3><p>Domestic, international, multi-city and everything between. We find the route that makes sense for you.</p><a href="#enquiry">Request a flight quote <ArrowUpRight size={15} /></a></div><div className="service-list">{[['Accommodation', 'Plan a stay'], ['Airport Transfers', 'Arrange a transfer'], ['Coach & Bus Hire', 'Hire a coach'], ['Corporate Travel', 'Move your team'], ['Group Travel', 'Plan group travel'], ['Holiday Packages', 'Explore holidays'], ['Tours & Experiences', 'Discover experiences']].map((item, i) => <a key={item[0]} href="#enquiry"><span>0{i + 2}</span><span className="service-list-copy">{item[0]}<small>{item[1]}</small></span><ArrowUpRight size={15} /></a>)}</div></div></section>
